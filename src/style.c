...
static void fixup_missing(yed_style *style) {
    ...
    if (style->good.flags == 0) {
        if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_16) {
            ATTR_SET_FG_KIND(style->good.flags, ATTR_KIND_16);
            style->good.fg = ATTR_16_GREEN;
        } else if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_256) {
            ATTR_SET_FG_KIND(style->good.flags, ATTR_KIND_256);
            style->good.fg = 2;
        } else if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_RGB ) {
            ATTR_SET_FG_KIND(style->good.flags, ATTR_KIND_RGB);
    ...
    if (style->bad.flags == 0) {
        if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_16) {
            ATTR_SET_FG_KIND(style->bad.flags, ATTR_KIND_16);
            style->bad.fg = ATTR_16_RED;
        } else if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_256) {
            ATTR_SET_FG_KIND(style->bad.flags, ATTR_KIND_256);
            style->bad.fg = 1;
        } else if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_RGB ) {
            ATTR_SET_FG_KIND(style->bad.flags, ATTR_KIND_RGB);
            style->bad.fg = RGB_32(127, 0, 0);
        }
    }

    if (style->active.flags == 0 || ATTR_FG_KIND(style->active.flags) == ATTR_KIND_16) {
        ATTR_SET_FG_KIND(style->white.flags, ATTR_KIND_16);
        style->white.flags     |= ATTR_16_LIGHT_FG;
        style->white.fg         = ATTR_16_GREY;
        ATTR_SET_FG_KIND(style->gray.flags, ATTR_KIND_16);
        style->gray.fg          = ATTR_16_GREY;
        ATTR_SET_FG_KIND(style->black.flags, ATTR_KIND_16);
        style->black.fg         = ATTR_16_BLACK;
        ATTR_SET_FG_KIND(style->red.flags, ATTR_KIND_16);
        style->red.fg           = ATTR_16_RED;
        ATTR_SET_FG_KIND(style->orange.flags, ATTR_KIND_16);
        style->orange.flags    |= ATTR_16_LIGHT_FG;
        style->orange.fg        = ATTR_16_RED;
        ATTR_SET_FG_KIND(style->yellow.flags, ATTR_KIND_16);
        style->yellow.fg        = ATTR_16_YELLOW;
        ATTR_SET_FG_KIND(style->lime.flags, ATTR_KIND_16);
        style->lime.flags      |= ATTR_16_LIGHT_FG;
        style->lime.fg          = ATTR_16_YELLOW;
        ATTR_SET_FG_KIND(style->green.flags, ATTR_KIND_16);
        style->green.fg         = ATTR_16_GREEN;
        ATTR_SET_FG_KIND(style->turquoise.flags, ATTR_KIND_16);
        style->turquoise.flags |= ATTR_16_LIGHT_FG;
        style->turquoise.fg     = ATTR_16_GREEN;
        ATTR_SET_FG_KIND(style->cyan.flags, ATTR_KIND_16);
        style->cyan.fg          = ATTR_16_CYAN;
        ATTR_SET_FG_KIND(style->blue.flags, ATTR_KIND_16);
        style->blue.fg          = ATTR_16_BLUE;
        ATTR_SET_FG_KIND(style->purple.flags, ATTR_KIND_16);
        style->purple.flags    |= ATTR_16_LIGHT_FG;
        style->purple.fg        = ATTR_16_BLUE;
        ATTR_SET_FG_KIND(style->magenta.flags, ATTR_KIND_16);
        style->magenta.fg       = ATTR_16_MAGENTA;
        ATTR_SET_FG_KIND(style->pink.flags, ATTR_KIND_16);
        style->pink.flags      |= ATTR_16_LIGHT_FG;
        style->pink.fg          = ATTR_16_MAGENTA;

        return;
    } else if (ATTR_FG_KIND(style->active.flags) == ATTR_KIND_256) {
        ATTR_SET_FG_KIND(style->white.flags, ATTR_KIND_256);
        style->white.fg = 231;
        ATTR_SET_FG_KIND(style->gray.flags, ATTR_KIND_256);
        style->gray.fg = 244;
        ATTR_SET_FG_KIND(style->black.flags, ATTR_KIND_256);
        style->black.fg = 16;
        ATTR_SET_FG_KIND(style->red.flags, ATTR_KIND_256);
        style->red.fg = 196;
        ATTR_SET_FG_KIND(style->orange.flags, ATTR_KIND_256);
        style->orange.fg = 214;
        ATTR_SET_FG_KIND(style->yellow.flags, ATTR_KIND_256);
        style->yellow.fg = 226;
        ATTR_SET_FG_KIND(style->lime.flags, ATTR_KIND_256);
        style->lime.fg = 154;
        ATTR_SET_FG_KIND(style->green.flags, ATTR_KIND_256);
        style->green.fg = 46;
        ATTR_SET_FG_KIND(style->turquoise.flags, ATTR_KIND_256);
        style->turquoise.fg = 43;
        ATTR_SET_FG_KIND(style->cyan.flags, ATTR_KIND_256);
        style->cyan.fg = 51;
        ATTR_SET_FG_KIND(style->blue.flags, ATTR_KIND_256);
        style->blue.fg = 21;
        ATTR_SET_FG_KIND(style->purple.flags, ATTR_KIND_256);
        style->purple.fg = 93;
        ATTR_SET_FG_KIND(style->magenta.flags, ATTR_KIND_256);
        style->magenta.fg = 127;
        ATTR_SET_FG_KIND(style->pink.flags, ATTR_KIND_256);
        style->pink.fg = 169;
...
#define ACTIVE_FACTOR (3)
    ...
    for (i = 0; i < N; i += 1) {
        ...
        if (ATTR_FG_KIND(attrs.flags) != ATTR_KIND_RGB) { continue; }
    ...
    for (i = 0; i < N; i += 1) {
        ...
        switch (set_scomps[i]) {
            #define __SCOMP(comp)                                       \
                case STYLE_##comp:                                      \
                    ATTR_SET_FG_KIND(style->comp.flags, ATTR_KIND_RGB); \
    ...
    if (style->white.flags == 0) {
        ATTR_SET_FG_KIND(style->white.flags, ATTR_KIND_RGB);
        style->white.fg = RGB_32(255, 255, 255);
    }
    if (style->gray.flags == 0) {
        ATTR_SET_FG_KIND(style->gray.flags, ATTR_KIND_RGB);
        style->gray.fg = RGB_32(127, 127, 127);
    }
    if (style->black.flags == 0) {
        ATTR_SET_FG_KIND(style->black.flags, ATTR_KIND_RGB);
        style->black.fg = RGB_32(0, 0, 0);
    }
}
...
void yed_set_default_styles(void) {
    ...
    ATTR_SET_FG_KIND(s.active.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.active.flags, ATTR_KIND_16);
    s.active.flags       |= ATTR_16_LIGHT_FG;
    s.active.fg           = ATTR_16_GREY;
    s.active.bg           = ATTR_16_BLACK;

    s.inactive            = s.active;

    ATTR_SET_FG_KIND(s.active_border.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.active_border.flags, ATTR_KIND_16);
    ...
    ATTR_SET_FG_KIND(s.selection.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.selection.flags, ATTR_KIND_16);
    s.selection.flags    |= ATTR_16_LIGHT_FG | ATTR_16_LIGHT_BG;
    s.selection.fg        = ATTR_16_GREY;
    s.selection.bg        = ATTR_16_BLACK;

    ATTR_SET_FG_KIND(s.search.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.search.flags, ATTR_KIND_16);
    s.search.fg           = ATTR_16_BLACK;
    s.search.bg           = ATTR_16_YELLOW;

    ATTR_SET_FG_KIND(s.search_cursor.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.search_cursor.flags, ATTR_KIND_16);
    s.search_cursor.fg    = ATTR_16_BLACK;
    s.search_cursor.bg    = ATTR_16_MAGENTA;

    ATTR_SET_FG_KIND(s.attention.flags, ATTR_KIND_16);
    s.attention.fg        = ATTR_16_RED;

    ATTR_SET_FG_KIND(s.associate.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.associate.flags, ATTR_KIND_16);
    s.associate.flags    |= ATTR_16_LIGHT_FG | ATTR_16_LIGHT_BG;
    s.associate.fg        = ATTR_16_BLUE;
    s.associate.bg        = ATTR_16_BLACK;

    s.command_line        = s.active;

    ATTR_SET_FG_KIND(s.status_line.flags, ATTR_KIND_16);
    ATTR_SET_BG_KIND(s.status_line.flags, ATTR_KIND_16);
    s.status_line.fg      = ATTR_16_BLACK;
    s.status_line.bg      = ATTR_16_BLUE;


    ATTR_SET_FG_KIND(s.code_comment.flags, ATTR_KIND_16);
    s.code_comment.flags |= ATTR_16_LIGHT_FG | ATTR_BOLD;
    s.code_comment.fg     = ATTR_16_YELLOW;

    ATTR_SET_FG_KIND(s.code_keyword.flags, ATTR_KIND_16);
    ...
    ATTR_SET_FG_KIND(s.code_fn_call.flags, ATTR_KIND_16);
    s.code_fn_call.fg     = ATTR_16_MAGENTA;

    ATTR_SET_FG_KIND(s.code_number.flags, ATTR_KIND_16);
    s.code_number.flags  |= ATTR_16_LIGHT_FG;
    s.code_number.fg      = ATTR_16_CYAN;

    s.code_constant       = s.code_number;

    ATTR_SET_FG_KIND(s.code_string.flags, ATTR_KIND_16);
