...
static inline void _yed_syntax_fixup_cache(yed_syntax *syntax, yed_buffer *buffer, _yed_syntax_cache *cache, int row) {
    _yed_syntax_cache_entry *it;
    int                      count;
...
out:;
    DBG("fixup: count = %d", count);
}

static inline void _yed_syntax_cache_rebuild(yed_syntax *syntax, yed_buffer *buffer, int row, int mod_event) {
    _yed_syntax_cache       *cache;
    yed_line                *line;
...
static inline void yed_syntax_buffer_mod_event(yed_syntax *syntax, yed_event *event) {
    _yed_syntax_cache_rebuild(syntax, event->buffer, event->row, event->buff_mod_event);
}
